...
function getMitarbeiter() {
  ...
  <input onChange={(e)=>{setSearch(e.target.value)}} type="text" className="form-control" placeholder="Suchen nach Nachname" aria-label="Suchen" aria-describedby="basic-addon2"/>
          ...
          {MitarbeiterArray
            ...
            <tr key={mitarbeiter.mitarbeiter_id} id={mitarbeiter.mitarbeiter_id}>
              ...
              <td>
                ...
                <Link to={ `/add-mitarbeiter/${mitarbeiter.mitarbeiter_id}`} className='btn btn-info action'><EditIcon/></Link>
